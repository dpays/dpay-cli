...
class Testcases(unittest.TestCase):
    ...
    def test_init(self):
        ...
        # keyword inits
        amount = Amount(amount=1.3, asset=Asset("BBD", dpay_instance=stm), dpay_instance=stm)
        self.dotest(amount, 1.3, symbol)

        # keyword inits
        amount = Amount(amount=1.3, asset=dict(Asset("BBD", dpay_instance=stm)), dpay_instance=stm)
        self.dotest(amount, 1.3, symbol)
    ...
    def test_json_appbase(self):
        ...
        amount = Amount("1", asset, new_appbase_format=False, dpay_instance=self.bts)
    ...
    def test_json_appbase2(self):
        asset = Asset("BBD", dpay_instance=self.bts)
        amount = Amount("1", asset, new_appbase_format=True, dpay_instance=self.bts)
        if self.bts.rpc.get_use_appbase():
            self.assertEqual(
                amount.json(),
