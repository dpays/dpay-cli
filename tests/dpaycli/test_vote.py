...
from dpaycli.vote import Vote, ActiveVotes, AccountVotes
from dpaycli.instance import set_shared_dpay_instance
from dpaycli.utils import construct_authorperm, resolve_authorperm, resolve_authorpermvoter, construct_authorpermvoter
...
class Testcases(unittest.TestCase):
    ...
    def test_vote(self):
        bts = self.bts
        vote = Vote(self.authorpermvoter, dpay_instance=bts)
        ...
        vote = Vote(self.voter, authorperm=self.authorperm, dpay_instance=bts)
        self.assertEqual(self.voter, vote["voter"])
        self.assertEqual(self.author, vote["author"])
        self.assertEqual(self.permlink, vote["permlink"])
        vote_json = vote.json()
    ...
    def test_keyerror(self, node_param):
        ...
        with self.assertRaises(
            ...
            Vote(construct_authorpermvoter(self.author, self.permlink, "asdfsldfjlasd"), dpay_instance=bts)

        with self.assertRaises(
            exceptions.VoteDoesNotExistsException
        ):
            Vote(construct_authorpermvoter(self.author, "sdlfjd", "asdfsldfjlasd"), dpay_instance=bts)

        with self.assertRaises(
            exceptions.VoteDoesNotExistsException
        ):
            Vote(construct_authorpermvoter("sdalfj", "dsfa", "asdfsldfjlasd"), dpay_instance=bts)

    def test_activevotes(self):
        bts = self.bts
        votes = ActiveVotes(self.authorperm, dpay_instance=bts)
        votes.printAsTable()
    ...
    def test_accountvotes(self):
        ...
        votes = AccountVotes(self.author, start=limit_time, dpay_instance=bts)
