...
from dpaycli.comment import Comment, RecentReplies, RecentByPath
...
class Testcases(unittest.TestCase):
    ...
    def test_comment(self):
        ...
        with self.assertRaises(
            ...
            Comment("@abcdef/abcdef", dpay_instance=bts)
        title = ''
        cnt = 0
        while title == '' and cnt < 5:
            c = Comment(self.authorperm, dpay_instance=bts)
            title = c.title
            cnt += 1
            if title == '':
                c.dpay.rpc.next()
    ...
    def test_comment_dict(self):
        ...
        cnt = 0
        while title == '' and cnt < 5:
            c = Comment({'author': self.author, 'permlink': self.permlink}, dpay_instance=bts)
            c.refresh()
            title = c.title
            cnt += 1
            if title == '':
    ...
    def test_recent_replies(self):
        bts = self.bts
        r = RecentReplies(self.author, skip_own=True, dpay_instance=bts)
        self.assertTrue(len(r) > 0)
        self.assertTrue(r[0] is not None)

    def test_recent_by_path(self):
        bts = self.bts
        r = RecentByPath(path="hot", dpay_instance=bts)
        self.assertTrue(len(r) > 0)
        self.assertTrue(r[0] is not None)
