...
import unittest
from dpaycligraphenebase.account import PrivateKey, PublicKey, Address
from dpaycligraphenebase.bip38 import encrypt, decrypt


key = {
    "public_key": u"DWB7jDPoMwyjVH5obFmqzFNp4Ffp7G2nvC7FKFkrMBpo7Sy4uq5Mj",
    ...
    "bts_address": u"DWB8DvGQqzbgCR5FHiNsFf8kotEXr8VKD3mR",
    ...
    "Uncompressed_BTC": u"DWBLAFmEtM8as1mbmjVcj5dphLdPguXquimn",
    "Compressed_BTC": u"DWBANNTSEaUviJgWLzJBersPmyFZBY4jJETY",
    "Uncompressed_PTS": u"DWBEgj7RM6FBwSoccGaESJLC3Mi18785bM3T",
    "Compressed_PTS": u"DWBD5rYtofD6D4UHJH6mo953P5wpBfMhdMEi",
}
...
class Testcases(unittest.TestCase):
    ...
    def test_btc_uncompressed(self):
        ...
        self.assertEqual(str(key["Uncompressed_BTC"]), (format(address.derive256address_with_version(0), "DWB")))

    def test_btc_compressed(self):
        public_key = PublicKey(key["public_key"])
        address = Address(address=None, pubkey=repr(public_key))
        self.assertEqual(str(key["Compressed_BTC"]), (format(address.derive256address_with_version(0), "DWB")))

    def test_pts_uncompressed(self):
        public_key = PublicKey(key["public_key"])
        address = Address(address=None, pubkey=public_key.unCompressed())
        self.assertEqual(str(key["Uncompressed_PTS"]), (format(address.derive256address_with_version(56), "DWB")))

    def test_pts_compressed(self):
        public_key = PublicKey(key["public_key"])
        address = Address(address=None, pubkey=repr(public_key))
        self.assertEqual(str(key["Compressed_PTS"]), (format(address.derive256address_with_version(56), "DWB")))
