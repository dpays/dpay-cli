...
from dpaycliapi.exceptions import NumRetriesReached, CallRetriesReached
...
class Testcases(unittest.TestCase):
    ...
    def setUpClass(cls):
        nodelist = NodeList()
        nodelist.update_nodes(dpay_instance=DPay(node=nodelist.get_nodes(normal=True, appbase=True), num_retries=3))
    ...
    def test_num_retries(self):
        with self.assertRaises(
            ...
            DPayNodeRPC(urls="https://wrong.link.com", num_retries=2, timeout=1)
        with self.assertRaises(
            NumRetriesReached
        ):
            DPayNodeRPC(urls="https://wrong.link.com", num_retries=3, num_retries_call=3, timeout=1)
        ...
        with self.assertRaises(
            ...
            DPayNodeRPC(urls=nodes, num_retries=0, num_retries_call=0, timeout=1)

    def test_error_handling(self):
        rpc = DPayNodeRPC(urls=self.nodes_appbase, num_retries=2, num_retries_call=3)
        with self.assertRaises(
            exceptions.NoMethodWithName
        ):
